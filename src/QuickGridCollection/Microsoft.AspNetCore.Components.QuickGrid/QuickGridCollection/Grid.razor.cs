...
namespace Microsoft.AspNetCore.Components.QuickGrid.QuickGridCollection
{
    ...
    public partial class Grid<TGridItem> : ComponentBase
    {
        /// <summary>
        ...
        /// <summary>
        /// Dernière valeur assignée à la propriété <see cref="Grid{TGridItem}.Items"/>.
        /// </summary>
        private ICollection<TGridItem>? _lastAssignedItems;
        private bool _collectingColumns;


        /// Le champ <see cref="Grid{TGridItem}.ClassAndStyle"/> est une instance de la classe <see cref="GridHtmlCssManager"/> qui permet de gérer les classes CSS et les styles 
        ...
        /// Lorsqu’une colonne est ajoutée à la grille via la méthode <see cref="Grid{TGridItem}.AddColumn(Column{TGridItem})"/>, 
        /// si elle est triable (c’est-à-dire si elle a une expression de propriété et que sa propriété IsSort est vraie), 
        /// elle est ajoutée au dictionnaire <see cref="Grid{TGridItem}.columnSortDirections"/> avec une direction de tri par défaut.
        /// Lorsqu’un utilisateur clique sur l’icône de tri d’une colonne, la méthode Sort <see cref="Grid{TGridItem}.ApplySort(Column{TGridItem})"/> est appelée. 
        /// Cette méthode vérifie si la colonne est triable et met à jour les listes <see cref="Grid{TGridItem}.columnsSortedAscending"/> et <see cref="Grid{TGridItem}.columnsSortedDescending"/> en fonction de la nouvelle direction de tri.
        /// Elle met également à jour la direction de tri dans le dictionnaire <see cref="Grid{TGridItem}.columnSortDirections"/>.
        /// La méthode <see cref="Grid{TGridItem}.GetSortClass(Column{TGridItem})"/>  est utilisée pour obtenir la classe CSS correspondant à la direction de tri d’une colonne. 
        /// Elle utilise le dictionnaire <see cref="Grid{TGridItem}.sortDirectionCssClasses"/> pour associer chaque direction de tri à sa classe CSS correspondante.

        /// <summary>
        /// Dictionnaire associant chaque colonne triable à sa direction de tri actuelle.
        /// </summary>
        private readonly Dictionary<Column<TGridItem>, SortDirection> columnSortDirections = new();

        /// <summary>
        /// Liste des colonnes à trier
        /// </summary>
        private readonly List<KeyValuePair<Column<TGridItem>, (SortDirection, Expression<Func<TGridItem, object>>)>> columnsSorted = new();
        /// <summary>
        ...
        /// </summary>
        private readonly List<KeyValuePair<Column<TGridItem>, Expression<Func<TGridItem, bool>>>> columnFilters = new();
        ...
        private readonly List<Column<TGridItem>> _columns;
        ...
        /// La propriété <see cref="Grid{TGridItem}.FilterSortChanged"/> est un callback de type
        ...
        /// l’objet <see cref="Grid{TGridItem}._gridFilteringSorting"/> contenant les informations de filtrage et de tri mises à jour.
        //
        /// <summary>
        ...
        /// Si la colonne est triable, elle est ajoutée au dictionnaire <see cref="Grid{TGridItem}.columnSortDirections"/> avec une direction de tri par défaut.
        /// </summary>
        ...
        /// </summary>
        internal async void ApplyColumnFilter(Expression<Func<TGridItem, bool>>? expression, Column<TGridItem> column)
        {
        ...
        /// Met à jour les listes orderBy <see cref="Grid{TGridItem}.columnsSortedAscending"/> et orderByDesc <see cref="Grid{TGridItem}.columnsSortedDescending"/> en fonction de la nouvelle direction de tri.
        /// Met également à jour la direction de tri dans le dictionnaire <see cref="Grid{TGridItem}.columnSortDirections"/>.
        /// </summary>
        private async void ApplySort(Column<TGridItem> column)
        {
        ...
        /// Utilise le dictionnaire sortClassValues <see cref="Grid{TGridItem}.sortDirectionCssClasses"/> pour associer chaque direction de tri à sa classe CSS correspondante.
        /// </summary>
        private string GetSortClass(Column<TGridItem> column)
        {
            if (column.PropertyExpression != null && column.IsSortable)
            {
        ...
        /// Utilise le dictionnaire sortClassValues <see cref="Grid{TGridItem}.sortDirectionCssClasses"/> pour associer chaque direction de tri à sa classe CSS correspondante.
        /// </summary>
        private string GetSortStyle(Column<TGridItem> column)
        {
            if (column.PropertyExpression != null && column.IsSortable)
            {
        ...
        private string AriaSortValue(Column<TGridItem> column)
        {
            var hasValue = columnSortDirections.TryGetValue(column, out SortDirection sortDirection);
            if (hasValue)
            {
        ...
        /// <summary>
        /// Invoque le callback <see cref="Grid{TGridItem}.RowSelected"/> avec l'élément de la ligne sélectionnée.
        /// </summary>
        ...
        /// Si la dernière valeur assignée à la propriété <see cref="Grid{TGridItem}.Items"/> n'est pas nulle,
